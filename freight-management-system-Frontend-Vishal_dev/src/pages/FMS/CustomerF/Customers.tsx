...
const Customer = () => {
    ...
    const validationSchema = Yup.object().shape({
        ...
        Tax_reg_no: Yup.number().typeError('Tax registration number must be a number').required('Tax registration number is required'),
    ...
    const saveUser = async () => {
        ...
        if (params.id) {
            ...
            params.credit_limit = 1000;
            let addUSer = await axios.post(`${config.API_BASE_URL}/user/customers`, params);
                            ...
                            >
                                <Dialog.Panel className="panel border-0 p-0 rounded-lg overflow-hidden w-full max-w-lg text-black dark:text-white-dark">
                                    ...
                                    <div className="text-lg font-medium bg-[#fbfbfb] dark:bg-[#121c2c] ltr:pl-5 rtl:pr-5 py-3 ltr:pr-[50px] rtl:pl-[50px]">
                                        {params.id ? 'Edit Contact' : 'Add Customer'}
                                    </div>
                                    <div className="p-5">
